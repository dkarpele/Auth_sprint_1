...
- [+] POST: auth/login: вход пользователя в аккаунт (обмен логина и пароля на пару токенов: JWT-access токен и refresh токен); 
- [-] GET[PUT]: auth/refresh: обновление access-токена;
- [+] POST: auth/logout: выход пользователя из аккаунта;
- [+] POST: users/change-[_]password:
  [+] POST: users/change-[_]login: изменение логина или пароля (с отправкой email вы познакомитесь в следующих модулях, поэтому пока ваш сервис должен позволять изменять личные данные без дополнительных подтверждений);
- [-] GET: users/login-[_]history: получение пользователем своей истории входов в аккаунт;
login_history(id:unique, login, source, login_time)
## API для управления доступами

- CRUD для управления ролями:
  - POST: roles/create: создание роли,
  - DELETE: roles/delete/<roles_id>: удаление роли,
  - PATCH roles/<roles_id>: изменение роли,
  - GET: roles/: просмотр всех ролей.
- POST: users/add-[_]role: назначить пользователю роль;
- DELETE: users/delete-[_]role: отобрать у пользователя роль;
- GET: users/roles : метод для проверки наличия прав у пользователя. 
